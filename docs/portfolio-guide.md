...
아닙니다. HMAC은 로컬 시연을 쉽게 재현하기 위한 방식입니다. 보호 릴리스에서는 ITSM 승인과 최종 증적을 Cosign으로 검증하고, 배포 러너에는 공개키만 제공합니다.

### 실제 금융권 경험으로 보아도 되나

아닙니다. 이 저장소는 금융 서비스에 필요한 안정성과 감사 통제를 기술 설계로 구현한 포트폴리오입니다. 예제 스캔 보고서와 모의 객체를 사용했으며, 실제 조직의 전자 결재, IdP, KMS, Kubernetes를 연동한 실적은 아닙니다.
