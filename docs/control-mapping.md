...
이 문서는 DevOps와 DevSecOps 통제를 구현한 코드와 검증 테스트를 정리합니다. 외부 시스템 연동 여부와 검증 범위도 함께 명시합니다.
...
| 스캔 실행 증명서 | SUP-01 | 보고서, 규칙 세트, 허용된 명령어 해시, 종료 코드, 커밋, 아티팩트, 취약점 DB, 러너, 서명자, 최신성 검증 | `finguard/attestation.py`, P0/P1 강화 테스트 |
...
| CB/SR 변경 통제 | CHG-01 | 커밋, 승인 역할, 롤백 계획, 배포 허용 시간, 최종 빌드 후 승인 | `finguard/change.py`, `tests/test_gate.py` |
...
| 배포 통제 | DEP-01 | PASS 증적, Cosign, 정책 ID와 버전 및 원문 해시, 증적 유효성, 클러스터와 워크로드 및 이미지가 승인 대상과 일치하는지 확인, RBAC 및 배포 허용 시간 검증 | `finguard/deployment.py`, 오래된 증적 회귀 테스트 |
...
- GitLab CI와 Jenkins가 같은 CLI를 사용하므로 정책 로직이 특정 CI 도구에 종속되지 않습니다.
- 판정 결과를 고정된 입력과 함께 서명하고, 정책 원문과 증적 유효성을 배포 시점에 다시 검증합니다. 승인된 배포 허용 시간 내에서도 롤아웃, 스모크 테스트 또는 결과 서명에 실패하면 정확한 이전 다이제스트로 되돌아가는 경로를 테스트했습니다.

## 외부 시스템 연동과 검증 범위

- 정책 판정과 배포 복구는 예제 보고서, 샘플 서비스와 임시 클러스터에서 검증합니다. 실제 조직의 ITSM, IdP와 KMS 연동은 별도 검증이 필요합니다.
...
- 예제 스캔 보고서는 파서 동작과 정책 판정을 재현하기 위한 입력입니다. 실제 스캐너 실행 결과와는 구분해 관리합니다.
